...
class PlayerAI(BaseAI):
    def getMove(self, grid):
        initial_state = State(grid, 0, time.clock(), None, 0)
        child, _ = self.maximize(initial_state, -inf, inf)
        return child.prev_move
    ...
    def chance(self, state, alpha, beta):
        if state.terminal_test():
            return None, state.eval()
        ch_child, ch_utility = None, 0
        for child in state.get_children_chance():
            ignore, utility = self.minimize(child, alpha, beta)
            if child.tile_value == 2:
                ch_utility += utility * 0.9
            elif child.tile_value == 4:
                ch_utility += utility * 0.1

        return None, ch_utility

class State:
    def __init__(self, grid, tile_value, start_time, move, depth):
        self.grid = grid
    ...
    def terminal_test(self):
        ...
        return time.clock() - self.start_time > 0.1 or not self.grid.canMove() and self.depth < 4
    ...
    def get_children_max(self):
        ...
        for move in self.grid.getAvailableMoves():
            ...
            gridCopy.move(move)
            children.append(State(gridCopy, 0, self.start_time, move, self.depth + 1))
        return children
    ...
    def get_children_min(self):
        ...
        for cell in self.grid.getAvailableCells():
            ...
            children.append(State(gridCopy, 0, self.start_time, None, self.depth + 1))
        return children

    def get_children_chance(self):
        children = []
        children.append(State(self.grid, 2, self.start_time, None, self.depth + 1))
        children.append(State(self.grid, 4, self.start_time, None, self.depth + 1))
        return children
    ...
    def eval(self):
        ...
        return (math.log(self.empty_cells()) if self.empty_cells() != 0 else 0) * empty_weight +\
            ...
            self.smoothness() * smooth_weight + self.max_value_at_corner() * corner_weight
...
if __name__ == '__main__':
    g = Grid()
    g.map = [[4, 0, 0, 2], [64, 16, 4, 0], [256, 128, 32, 8], [512, 256, 64, 16]]
    displayer = Displayer()
    displayer.display(g)
    s = State(g, 2, 0, None, 0)
    print(s.smoothness())
